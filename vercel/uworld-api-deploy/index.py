...
_ENHANCED_EXPLANATIONS_PATH = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "nbme120_enhanced_explanations.json"
)
try:
...
_OE_EXPLANATIONS_PATH = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "test2_openevidence_explanations.json"
)
try:
...
def _performance_badge(row, oe, median_time, fast_threshold, slow_threshold):
    ...
    pct_phrase = (
        f"only ~{pct}% of test-takers get this right"
        if (pct is not None and pct <= 55)
        else (f"~{pct}% get this right" if pct is not None else None)
    )
    ...
    if is_correct and is_hard:
        return {
            "tone": "celebrate",
            "emoji": "🌟",
            "title": "Clutch — hard item nailed",
            "text": "You locked in a genuinely tough question"
            + (f" — {pct_phrase}." if pct_phrase else ".")
    ...
    if is_correct:
        return {
            ...
            "title": "Solid",
            "text": "Correct."
            + (f" {pct_phrase[0].upper() + pct_phrase[1:]}." if pct_phrase else ""),
        }
    if is_wrong and is_easy and fast:
        return {
            "tone": "careless",
            "emoji": "⏱️",
            "title": "Slow down — avoidable miss",
    ...
    if is_wrong and is_hard:
        return {
            "tone": "tough",
            "emoji": "💪",
            "title": "Hard item — expected stretch",
            "text": "A legitimately difficult question"
            + (f" ({pct_phrase})." if pct_phrase else ".")
    ...
    if is_wrong and slow:
        return {
            "tone": "pace",
            "emoji": "🐢",
            "title": "Long deliberation, still missed",
...
def auth_response(user, status=200):
    ...
    response = make_response(
        jsonify({"token": token, "user": public_user(user)}), status
    )
    response.set_cookie(
        "token", token, httponly=True, samesite="Lax", secure=True, max_age=604800
    )
    return response
...
def _unsafe_question_response(question_id):
    return jsonify(
        {
            "error": "Question is not safe for exam delivery or scoring",
...
def qbank_get_question(test_id, question_idx):
    ...
    question = get_question_by_id(question_id, require_exam_safe=True)

    if not question:
        raw_question = get_question_by_id(question_id)
        if raw_question:
    ...
    return jsonify({"index": question_idx, "total": total, "question": safe_question})
    ...
    return jsonify(
        {
            ...
            "blocks": json.loads(test_session["block_info"])
            if test_session.get("block_info")
            else [],
            "answers": answers,
        }
    )
...
def _insight_tone(is_correct, time_spent, median_time):
    if (
        _is_correct_value(is_correct)
        and time_spent is not None
        and time_spent <= max(12, int(median_time * 0.45))
    ):
        return {
            "label": "Clean hit",
            "style": "green",
    ...
    if (
        _is_correct_value(is_correct)
        and time_spent is not None
        and time_spent >= max(90, int(median_time * 1.6))
    ):
        return {
...
def _coaching_note(row, explanation, median_time):
    ...
    if _is_wrong_value(row.get("isCorrect")) and time_spent <= max(
        12, int(median_time * 0.45)
    ):
    ...
    if _is_correct_value(row.get("isCorrect")) and time_spent >= max(
        90, int(median_time * 1.6)
    ):
...
def _build_review_summary(mode, rows):
    ...
    def aggregate(key, limit=5):
        ...
        for row in answered_rows:
            ...
            bucket = buckets.setdefault(
                name, {"name": name, "correct": 0, "answered": 0}
            )
            bucket["answered"] += 1
        ...
        strong = [
            ...
            for item in sorted(
                values,
                key=lambda item: (-item["accuracy"], -item["answered"], item["name"]),
            )
            if item["correct"] > 0
        ][:limit]
        focus_candidates = [item for item in values if item["missed"] > 0]
        focus = sorted(
            focus_candidates or values,
            key=lambda item: (
                item["accuracy"],
                -item["missed"],
                -item["answered"],
    ...
    for row in rows:
        ...
        row["isMediaQuestion"] = bool(
            row.get("imageUrls") or row.get("tables") or row.get("optionTable")
        )
        ...
        row["insightTone"] = _insight_tone(
            row.get("isCorrect"), row.get("timeSpent"), median_time
        )
        row["coachingNote"] = _coaching_note(row, explanation, median_time)
        badge = _performance_badge(
            row, row.get("oeReport"), median_time, fast_threshold, slow_threshold
        )
                ...
                celebrations.append(
                    {
                        ...
                        "title": (row.get("oeReport") or {}).get("title")
                        or row.get("subject")
                        or "Hard item",
                        "pctCorrect": _pct_correct_from_pvalue(
                            ((row.get("oeReport") or {}).get("metrics") or {}).get(
                                "pValue"
                            )
                        ),
                    }
        ...
        if row.get("block"):
            block = blocks.setdefault(
                row["block"],
                {"block": row["block"], "correct": 0, "answered": 0, "total": 0},
            )
            block["total"] += 1
            if row.get("isCorrect") is not None:
                block["answered"] += 1
    ...
    if block_stats:
        ...
        second_half = (
            block_accuracies[max(1, len(block_accuracies) // 2) :]
            or block_accuracies[-1:]
        )
    ...
    readiness = round(
        min(
            ...
            + 0.2 * completion
            + 0.15 * stability
            + 0.1 * max(media["accuracy"], text_only["accuracy"]),
        ),
        1,
    )
    ...
    study_priorities = sorted(
        actionable_priorities or study_priorities,
        key=lambda item: (
            item["accuracy"],
            -item["missed"],
            -item["answered"],
    ...
    conversational_headline = (
        ...
        + (
            ...
            if tone == "encouraging"
            else "Some pieces are working, but the weak spots are still expensive."
            if tone == "mixed"
            else "The misses cluster into a repairable set of patterns, which is better than chaos."
        )
    )
    ...
    return {
        ...
        "celebrations": sorted(
            celebrations,
            key=lambda c: c["pctCorrect"] if c["pctCorrect"] is not None else 999,
        )[:8],
...
def qbank_test_review(test_id):
    ...
    question_ids = json.loads(test_session["question_ids"])
    answers_by_question = {
        answer["question_id"]: answer for answer in get_test_answers(test_id)
    }
    rows = []
    for idx, question_id in enumerate(question_ids):
        ...
        rows[-1]["oeReport"] = (
            get_openevidence_report(question_id)
            if test_session["mode"] == "test2"
            else None
        )
...
def stats_overview():
    ...
    for subject, stat in subject_stats.items():
        stat["accuracy"] = (
            (stat["correct"] / stat["answered"] * 100) if stat["answered"] > 0 else 0
        )
...
def serve_image(form, filename):
    return send_from_directory(
        os.path.join(os.path.dirname(__file__), "images_webp", form), filename
    )
...
def qbank_browse():
    ...
    questions = load_questions()
    include_unsafe = (request.args.get("include_unsafe") or "").strip().lower() in (
        "1",
        "true",
        "yes",
    )
    ...
    def matches(q):
        if not include_unsafe and not is_exam_safe_question(q):
            return False
        ...
        if (
            difficulty
            and (q.get("difficulty_band") or q.get("difficulty")) != difficulty
        ):
            return False
    ...
    def shape(q):
        ...
        return {
            ...
            or ([q["image_url"]] if q.get("image_url") else []),
...
def serve_cropped_image(filename):
    response = send_from_directory(
        os.path.join(os.path.dirname(__file__), "images_crop"),
        filename,
        mimetype="image/webp",
    )
...
def serve_page_image(form, filename):
    response = send_from_directory(
        os.path.join(os.path.dirname(__file__), "images_pages", form),
        filename,
        mimetype="image/webp",
    )
    response.headers["Cache-Control"] = "public, max-age=31536000, immutable"
